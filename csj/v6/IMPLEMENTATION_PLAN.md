...
本文件第 2-14 节保留 V6 当时预注册的风险目标、风险头、校准与 overlay 设计，用于解释 V6 为什么停止；它们不是继续实施 V6 P1-P5 的授权。

V7 已按实际数据完成以下迁移：

- 将训练/诊断池从少量品种扩展为纯覆盖度规则冻结的 21 品种，`i/jm/rb` 保持正式业务门禁；
- 将 split key 从 target completion day 改为 `origin_trading_day`，并固定研究起点为 `2025-10-15`；
- 保留三日不利波动标签、fit-only P80、五折跨度和全部事件门槛；
- 权威 P0 运行 `v7_p0_20260813_verified` 已通过，V6 P0 仍保持失败。

P1-P5 的 V7 实施不得直接照搬本文件中的 V6 路径、品种角色或命令。当前唯一权威交接方案是 [`../v7/IMPLEMENTATION_PLAN.md`](../v7/IMPLEMENTATION_PLAN.md)，其中已按 V7 的真实 case 数、产品角色、origin-day folds、路径缓存去重和正式 evaluation 封存做了调整。
